...
unsigned int nStakeMinAge = 20 * 24 * 60 * 60; // 20 days as zero time weight
unsigned int nStakeMaxAge = 2000000000; // No MAX
int64_t nTargetSpacing = 15 * 60;  // Time between reward:    15 minutes
...
const string strMessageMagic = "BigsCoin Signed Message:\n";
...
// miner's coin base reward premine -- Premined: 23.000.000
int64_t GetProofOfWorkReward(int64_t nFees)
{
        int64_t nSubsidy = 1 * COIN;
        
        if (pindexBest->nHeight+1 < 2)
        	nSubsidy = 23000000 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BigsCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BigsCoin.com Block";
        CTransaction txNew;
        txNew.nTime = 1514960425;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1514960425;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2530030;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc783d45fa82de1a147f7d715e1c901a70295f2e9148fc93b922855d36726db63"));
        block.print();
